...
def cutting_stock(raw_length: int, final_to_demands: dict[int,int]) -> list[tuple[Pattern,int]]:
    ...
    objective = [waste_for_pattern(raw_length, pattern) for pattern in patterns] + [0.5 * final for final in finals]
    ...
    for final,demand in final_to_demands.items():
       ...
       inequality_constraints_matrix.append([-pattern[final] for pattern in patterns] + [f == final for f in finals])
    ...
    result = linprog(
        ...
        b_ub = inequality_constraints_vector,
        integrality = [1 for _ in patterns] + [0 for _ in finals]
    )
