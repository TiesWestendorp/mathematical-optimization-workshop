...
def cutting_stock(raw_length: int, final_demands: dict[int,int]) -> list[tuple[dict[int,int],int]]:
    ...
    b_eq = []
    for final in finals:
        A_eq.append([-pattern[final] for pattern in possible_patterns] + [f == final for f in finals])
        b_eq.append(-final_demands[final])
    
    result = linprog(
        waste_per_pattern + over_production_per_final,
        A_eq = A_eq,
        b_eq = b_eq,
        integrality = ones + [0 for _ in finals])
